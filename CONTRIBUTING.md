...
- **Node 10 LTS:** `@cubedevinc/config` has a minimum Node version
  requirement of 10.0.0. Pull requests must not require a Node version greater
...
``` bash
git clone https://github.com/cubedevinc/config.git
cd config
yarn
yarn test
```
...
To release a new version, simply merge into `master` and let the CI
take care of everything. This includes Git tags, changelog generation and NPM
releases.

## Reporting a security vulnerability

We want to ensure that `@cubedevinc/config` is secure for everyone. If
...
you've discovered a security concern, please email us at hello@cube.dev
